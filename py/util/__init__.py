from .textfilter import filt_message
...
async def silence(ev: GroupMessageEvent, ban_time: int, skip_su: bool = True):
    try:
        ...
        bot = get_bots().get(str(ev.self_id))
        if isinstance(bot, Bot):
            await bot.set_group_ban(group_id=ev.group_id, user_id=ev.user_id, duration=ban_time, self_id=ev.self_id)
    except ActionFailed as e:
...
def split_list(l: list, length: int):

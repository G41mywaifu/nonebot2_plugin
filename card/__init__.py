...
path ='C:\\nb2\\mimibot\\src\\plugins\\card\\assets\\' #请修改为自己的assets path，注意windows斜杠转义

sv = Service('tarot',  enable_on_default= True)

@sv.on_startswith(('塔罗牌'))
async def send_playerInfo(bot, event: MessageEvent):
    await bot.send(event,'请稍等，正在洗牌中  如需自选卡牌请在指令后加四个数字(1-78) 空格间隔 第一个数字不要与塔罗牌留空') 
    q = event.message.extract_plain_text().strip().split('塔罗牌')[1].split(' ')
    ...
    if  len(q)==4 :
        try:
            ...
            for i in q:
                if int(i)>78:
                    await bot.send(event,'数字不得大于78')
                    return
                indices.append(int(i))
                
        except:
            await bot.send(event,'参数有误')
            return
    else:
        await bot.send(event,'抽卡数量不为4  将使用系统抽卡')
    ...
    for count in range(4):
       ...
       # sv.logger.info(msg)
        if count < 3:
            await bot.send(event, "".join(msg)+R.img(f'assets//{name}.jpg').cqcode, at_sender=True)
        else:
            await bot.send(event, "".join(msg)+R.img(f'assets//{name}.jpg').cqcode, at_sender=True)
